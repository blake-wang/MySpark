...
/**
  * Created by wanglei on 2017/6/3.
  */
object IPLocation {
  val data2MySQL = (iterator: Iterator[(String, Int)]) => {
    ...
    try {
      conn = DriverManager.getConnection("jdbc:mysql://192.168.33.21:3306/bigdata", "root", "hadoop")
  ...
  def main(args: Array[String]) {
    ...
    val ipRulesRdd = sc.textFile("h://ip.txt").map(line => {
    ...
    //加载要处理的数据
    val ipsRDD = sc.textFile("H://access_log").map(line => {
      val fields = line.split("\\|")
      fields(1)
    })
